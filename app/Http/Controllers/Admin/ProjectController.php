...
class ProjectController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Handle checkboxes - se não enviados, são false
        $validated['featured'] = $request->has('featured') ? (bool)$request->featured : false;
        $validated['active'] = $request->has('active') ? (bool)$request->active : false;
        ...
        if ($request->hasFile('image')) {
            try {
                ...
                if (!$imagePath) {
                    throw new \Exception('Falha ao fazer upload da imagem principal');
                }
                $validated['image'] = $imagePath;
            } catch (\Exception $e) {
                return back()->withInput()->withErrors(['image' => 'Erro ao fazer upload da imagem: ' . $e->getMessage()]);
            }
        }
        ...
        if ($request->hasFile('gallery')) {
            try {
                ...
                foreach ($request->file('gallery') as $index => $file) {
                    ...
                    if (!$galleryPath) {
                        throw new \Exception("Falha ao fazer upload da imagem {$index} da galeria");
                    }
                    $galleryPaths[] = $galleryPath;
                }
                $validated['gallery'] = $galleryPaths;
            } catch (\Exception $e) {
                // Se houver erro na galeria, deletar imagem principal se foi salva
                if (isset($validated['image'])) {
                    Storage::disk('public')->delete($validated['image']);
                }
                return back()->withInput()->withErrors(['gallery' => 'Erro ao fazer upload da galeria: ' . $e->getMessage()]);
            }
        } else {
        ...
        try {
            ...
            return back()->withInput()->withErrors(['error' => 'Erro ao salvar projeto: ' . $e->getMessage()]);
        }
