        ...
        @if ($errors->any())
            <div class="mb-6 bg-red-50 border border-red-200 text-red-800 px-4 py-3 rounded-lg">
                <div class="flex items-center mb-2">
                    ...
                    <strong>Erro ao salvar o projeto:</strong>
                </div>
        ...
        <form action="{{ route('admin.projects.store') }}" method="POST" enctype="multipart/form-data" class="bg-white shadow rounded-lg p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="title" class="form-input w-full {{ $errors->has('title') ? 'border-red-500' : '' }}" value="{{ old('title') }}" required>
                    @error('title')
                ...
                <div>
                    ...
                    <input type="text" name="technologies" class="form-input w-full {{ $errors->has('technologies') ? 'border-red-500' : '' }}" value="{{ old('technologies') }}" required>
                    @error('technologies')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div class="md:col-span-2">
                    ...
                    <textarea name="short_description" class="form-textarea w-full {{ $errors->has('short_description') ? 'border-red-500' : '' }}" rows="2" required>{{ old('short_description') }}</textarea>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" class="form-textarea w-full {{ $errors->has('description') ? 'border-red-500' : '' }}" rows="6" required>{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="status" class="form-select w-full" required>
                        ...
                        <option value="planned" {{ old('status') == 'planned' ? 'selected' : '' }}>Planejado</option>
